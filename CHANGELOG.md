...
- 改动内容：说明「用户模型调用情况」表头可排序列为 `<button>`，点击后浏览器会为该按钮绘制默认焦点环（视觉上像蓝色方框）；为 `.admin-usage-sort-th` 关闭默认 `outline`/`box-shadow`，并在键盘 `:focus-visible` 时用浅色下划线替代粗框，避免干扰阅读且保留基本键盘可达性提示。
