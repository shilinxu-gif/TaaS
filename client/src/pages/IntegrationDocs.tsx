  ...
  const snippets = useMemo<Record<SdkTabId, SnippetEntry>>(
    () => ({
      "openai-python": {
        ...
        summary: text(
          "最推荐的迁移方式之一对已经使用 OpenAI Python SDK 的项目，几乎只需要改 `base_url` 和 `api_key`",
...
print(resp.model_dump_json(indent=2))`,
      ...
      "openai-node": {
        ...
        summary: text(
          "适合 Node.js 服务端或 BFF你可以沿用现有 OpenAI 调用形态，只切换网关地址与密钥",
...
print(resp.model_dump_json(indent=2))`,
      ...
      "java-http": {
        ...
        summary: text(
          "适合 Java 后端服务集成若 AppKey 已绑定模型，可直接省略 `model`，由网关自动选择",
...
print(resp.model_dump_json(indent=2))`,
      ...
      "gemini-node": {
        ...
        title: text("OpenAI 兼容形态（Gemini 模型名）", "OpenAI-compatible shape (Gemini model id)"),
        summary: text(
          "与 OpenAI 系相同的 `baseURL` 与路径；将 `model` 换成 `gemini-*` 即可无独立 `/v1/gemini/...` 入口",
  ...
  const faqItems = [
    {
      ...
      answer: text(
        "因为当前控制台 JWT 和网关调用是两套认证边界JWT 用于控制台管理接口；AppKey 用于统一 Base URL `https://www.itoken.group` 及网关调用手写 HTTP 时：OpenAI 与 Gemini 共用 `.../gateway/v1/chat/completions`，Claude 使用 `.../gateway/v1/messages`",
    ...
    },
    {
      ...
      answer: text(
        "不建议生产环境应通过你自己的服务端中转，以避免密钥暴露、预算失控和来源不可控",
    ...
    },
    {
      ...
      answer: text(
        "会租户在控制台关闭缓存后，网关将跳过幂等响应缓存的读与写（仍保留 QPS/预算等逻辑）；`ttl_seconds` 会作为 Redis 中幂等响应条目的过期时间语义/混合模式当前与精确模式一致，仅幂等缓存，不做向量相似命中",
    ...
    },
    {
      ...
      answer: text(
        "可以（在租户缓存开启且网关未关闭该能力时）对 **非流式** `chat/completions`，若无幂等键，网关会对白名单内的请求字段（如 model、messages、temperature 等）做稳定规范化后 SHA-256，使用 Redis 键前缀 `bodyfp:`；**幂等键始终优先**流式 `stream: true` 当前不走该指纹缓存可通过环境变量 `TAAS_GATEWAY_BODY_FINGERPRINT_CACHE=false` 全局关闭",
    ...
    },
    {
      ...
      answer: text(
        "模型存在不等于该 AppKey 有权限使用请同时检查当前密钥的模型白名单、供应商配置是否完成，以及该模型是否处于可用状态",
    ...
    },
    {
      ...
      answer: text(
        "可以如果 AppKey 只绑定了一个模型，网关会自动使用该模型；如果 AppKey 绑定了多个允许模型，网关会在当前可用模型中随机选择如果 AppKey 没有限定模型范围，仍建议显式传 model",
    ...
    <div className="integration-page">
      ...
      >
        <div className="integration-hero-copy">
          ...
          <p>
            {text(
              "OpenAI SDK、Anthropic SDK、Claude Code 及多数兼容客户端都填根域名即可完整 HTTP path（`chat/completions` 与 `messages`）按下方「供应商族」分块查看，避免多 URL 并列混淆",
        ...
        <div className="integration-hero-panel integration-hero-panel--single">
          ...
          <article className="integration-hero-json-card">
            ...
            <p className="muted integration-hero-json-note">
              {text(
                "环境变量名因工具版本可能不同；将占位密钥换成你的 AppKeyAnthropic 系完整 path 见「Claude 系」一节",
        ...
        <article className="integration-card">
          ...
          <ol className="integration-step-list">
            ...
            <li>
              ...
              <span>
                {text(
                  "OpenAI SDK / Anthropic SDK / 多数兼容客户端：只改 Base URL（根域名）与 AppKeyGemini 在本平台仍用 OpenAI SDK 的 `chat.completions`原生 HTTP 再按族选择 path",
            ...
            <li>
              ...
              <span>
                {text(
                  "`https://www.itoken.group` 是统一 Base URL手写 HTTP 时：OpenAI 与 Gemini 均 POST `.../gateway/v1/chat/completions`；仅 Claude POST `.../gateway/v1/messages`",
        ...
        <article className="integration-card integration-card--wide">
          <div className="integration-section-head">
            <div>
              ...
              <p className="muted">
                {text(
                  "`system` 负责描述模型扮演的角色与回答规则，`user` 才是真正的问题输入若只有 `system` 没有 `user`，不同上游模型可能表现不一致",
